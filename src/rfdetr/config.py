...
"""
文件说明：本文件集中定义 RF-DETR 当前版本使用的模型配置与训练配置。
功能说明：负责把模型结构参数、训练超参数和常用兼容字段统一收敛到 Pydantic
配置对象中，供训练入口、推理入口和兼容层共同复用。
...
class ModelConfig(BaseConfig):
    ...
    兼容说明：
    - 当前主线字段服务于新版 `src/rfdetr`。
    - `use_white / fusion_*` 保留给当前仓库的双模态 custom 兼容层使用；
      单模态主线不会主动消费这些字段，但保留它们可以避免每次上游更新后
      都在 custom 入口重复手工拼接一份“半官方配置”。
    """
...
class TrainConfig(BaseModel):
    ...
    兼容说明：
    - 新版 PTL 训练主线主要消费通用字段。
    - `dropout / resume_load_lr_scheduler / use_white / fusion_*` 继续保留，
      是为了兼容当前仓库尚未迁移走的旧式 custom 训练入口。
    """
    ...
    resume_load_lr_scheduler: bool = True
    use_white: bool = True
    fusion_type: Literal["none", "uv_queries_white"] = "uv_queries_white"
    fusion_num_heads: int = 8
    fusion_num_layers: int = 4
