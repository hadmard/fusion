...
"""包内兼容辅助模块：把 Pydantic 配置转换为旧式 argparse namespace。

文件说明：本文件负责把新版 `ModelConfig / TrainConfig` 组合成一份旧接口仍能
消费的 `types.SimpleNamespace`。
功能说明：在不把整个项目重新退回到旧版参数系统的前提下，为仍然依赖
`args.xxx` 风格读取配置的模块提供一个过渡层。

结构概览：
  第一部分：导入依赖
  第二部分：`build_namespace` 实现
"""
...
def build_namespace(model_config: ModelConfig, train_config: TrainConfig) -> Any:
    ...
    return types.SimpleNamespace(
        ...
        lazystrike_enabled=mc.lazystrike_enabled,
        lazystrike_topk=mc.lazystrike_topk,
        lazystrike_topk_ratio=mc.lazystrike_topk_ratio,
        lazystrike_sigma_scale=mc.lazystrike_sigma_scale,
        lazystrike_score_numerator=mc.lazystrike_score_numerator,
        lazystrike_apply_to=mc.lazystrike_apply_to,
        ...
        resume_load_lr_scheduler=tc.resume_load_lr_scheduler,
        lazystrike_loss_coef=tc.lazystrike_loss_coef,
        encoder_only=False,
        backbone_only=False,
        use_cls_token=False,
        lr_scheduler=tc.lr_scheduler,
        lr_min_factor=tc.lr_min_factor,
