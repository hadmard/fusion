...
"""
文件说明：
    本模块提供模型导出与部署相关的工具，包括：
        - 将模型导出为 ONNX（`export_onnx`）
        - 简化 ONNX（`onnx_simplify`）
        - 使用 trtexec 生成 TensorRT 引擎的包装/解析工具

结构概览：
    - 导入与依赖设置
    - 导出与简化函数（`export_onnx`, `onnx_simplify`）
    - trtexec 调用包装与输出解析工具

注意：仅添加中文模块说明，不改变任何导出实现或外部工具调用逻辑。
"""
import os
import random
import re
