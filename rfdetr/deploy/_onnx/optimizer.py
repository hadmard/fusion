...
"""
文件说明：
    ONNX 优化器实现，用于对导出的 ONNX 图进行常见变换（如常量折叠、Resize 修复、InstanceNorm 分解等），以提高 TensorRT/ONNX Runtime 的兼容性与性能。

结构概览：
    - `OnnxOptimizer` 类：加载/保存 ONNX、执行公共优化（`common_opt`）、并包含多种图变换函数（`resize_fix`, `decompose_instancenorms` 等）。
    - 使用 `CustomOpSymbolicRegistry` 注册的优化器可被 `common_opt` 自动调用。

注意：仅添加中文说明，不修改优化器实现逻辑。
"""
import os
