...
"""
文件说明：
    本工具用于对导出的 ONNX / TensorRT 引擎进行推理基准测试（latency / throughput），并可选择使用 COCO 验证集进行效果评估。

结构概览：
    - 命令行参数解析（`parser_args`）
    - ONNX / TensorRT 运行与计时工具（trt/ORT 的包装）
    - COCO 评估适配（`CocoEvaluator`）和结果汇总函数

注意：此文件与导出并行，运行依赖多种第三方库（onnxruntime / tensorrt / pycuda / pycocotools），仅添加中文说明不修改实现。
"""
import argparse
import contextlib
import copy
