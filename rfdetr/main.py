...
"""
文件说明：
    本文件为 RF-DETR 的主入口与模型封装层，负责：
        - 解析并封装模型构建参数
        - 管理预训练权重的下载与加载策略
        - 根据配置选择单模态或双模态模型构建器并实例化
        - 提供模型导出与优化（JIT/ONNX）辅助方法

结构概览：
    第一部分：导入依赖（第三方库与内部模块）
    第二部分：全局常量与模型权重映射（OPEN_SOURCE_MODELS / HOSTED_MODELS）
    第三部分：辅助函数（download_pretrain_weights 等）
    第四部分：`Model` 封装类（构造、权重加载、导出、LoRA 等）

代码段说明：
    - 第三部分中的辅助函数仅处理 I/O 与权重选择，不包含模型前向逻辑。
    - `Model` 类为轻量封装，构造时会根据 `dual_modal` 参数选择 `custom.dual_model` 或 `rfdetr.models.build_model`。
    - 所有改动仅为注释添加或说明性文本；未修改任何执行逻辑。
"""
import argparse
import ast
import copy
...
class Model:
    def __init__(self, **kwargs):
        ...
        if args.dual_modal:
            ...
            self.model = build_dual_model(args)
        else:
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        if args.resume:
            ...
            # strict=False：允许 checkpoint 缺少新增参数（如 alpha_attn/alpha_ffn），
            # 这些参数初值为 0，缺失时保持模型默认初始化即可，不影响训练。
            missing, unexpected = model_without_ddp.load_state_dict(checkpoint['model'], strict=False)
            if missing:
                print(f"[Resume] 以下键在 checkpoint 中缺失，将使用模型默认初始化：{missing}")
            if unexpected:
                print(f"[Resume] checkpoint 中存在模型未定义的键（已忽略）：{unexpected}")
            if args.use_ema:
            ...
            if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
                ...
                if getattr(args, "resume_load_lr_scheduler", True):
                    lr_scheduler.load_state_dict(checkpoint['lr_scheduler'])
                else:
                    resumed_steps = args.start_epoch * num_training_steps_per_epoch_lr
                    lr_scheduler.last_epoch = resumed_steps - 1
                    if hasattr(lr_scheduler, "_step_count"):
                        lr_scheduler._step_count = max(1, resumed_steps)
...
def get_args_parser():
    ...
    parser.add_argument('--dual_modal', action='store_true', help='使用白光+UV双模态训练')
    parser.add_argument(
        '--use_white',
        ...
        nargs="?",
        const=True,
        help='Enable the auxiliary White branch. Set to false for UV-only baseline ablations.',
    )
    parser.add_argument(
        '--fusion_type',
        type=str,
        default='uv_queries_white',
        choices=('none', 'uv_queries_white'),
        help='Cross-modal fusion type. Use "none" for UV-only baseline.',
    )
    parser.add_argument(
        '--fusion_num_heads',
        default=8,
        type=int,
        help='Number of heads in the UV<-White cross-attention block.',
    )
    parser.add_argument(
        '--fusion_num_layers',
        default=1,
        type=int,
        help='Number of stacked UV<-White fusion blocks per feature level.',
    )
