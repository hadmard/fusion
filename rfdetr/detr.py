...
"""
文件说明：
  本文件定义了对外的高层 `RFDETR` 类以及与用户交互的训练/推理/导出工具，主要负责：
    - 提供模型配置解析与实例化入口（`RFDETR` 类）
    - 提供数据集类名解析工具（COCO/YOLO 格式检测）
    - 封装模型训练、导出、优化等上层 API

结构概览：
  第一部分：导入依赖（包含第三方库与内部模块）
  第二部分：配置类导入与辅助常量（如 `COCO_CLASSES`）
  第三部分：`RFDETR` 高层类（初始化、训练入口、导出等方法）

使用说明：
  - `RFDETR` 类为面向用户的封装，典型用法为 `model = RFDETR(num_classes=...)` 后调用 `model.train()` 或 `model.predict()`。
  - 本次修改仅在文件开头增加中文说明与结构概览注释，未更改任何功能性代码。
"""
