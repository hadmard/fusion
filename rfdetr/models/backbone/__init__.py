...
"""
文件说明：
    backbone 包的入口，封装了构建 backbone 与位置编码的工具，并提供 `Joiner` 用于将 backbone 与位置编码串联成一个模块。

结构概览：
    - `Joiner`：组合 backbone 与位置编码的轻量封装，支持导出模式
    - `build_backbone`：传入各类配置参数后构建并返回 `Joiner(backbone, position_embedding)`

注意：此处仅增加中文说明，保留原始实现与导出接口。
"""
