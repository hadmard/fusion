...
"""
文件说明：
    本模块实现 Backbone 的高层封装，负责根据配置选择具体的 encoder（如 DINOv2）并将其与 `MultiScaleProjector` 等投影器结合，输出用于 Transformer 的多尺度特征与位置编码。

结构概览：
    - `Backbone` 类：封装 encoder、projector、输出通道与特征索引的构建逻辑
    - 通过 `Backbone` 的 `forward` 返回特征序列与 masks，供 `Joiner` 或上层构建器使用

注意：仅增加模块级说明注释，未修改任何实现。
"""
import torch
