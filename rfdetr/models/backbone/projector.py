...
"""
文件说明：
    本模块实现用于将 backbone 特征投射/填充为多尺度金字塔特征的投影器（MultiScaleProjector），并包含若干卷积与瓶颈等轻量模块。

结构概览：
    - 归一化与激活帮助器：`LayerNorm`, `get_norm`, `get_activation`
    - 卷积块与瓶颈实现：`ConvX`, `Bottleneck`, `C2f`
    - `MultiScaleProjector`：构建多尺度特征金字塔的顶层实现

注意：仅添加中文说明，不更改任何函数实现。
"""
import numpy as np
import torch
