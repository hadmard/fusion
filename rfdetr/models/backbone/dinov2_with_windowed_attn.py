...
"""
文件说明：
    本文件包含对 DINOv2 带窗口注意力（windowed attention）变体的模型定义与配置封装，该实现基于 HuggingFace Transformers 的 DINOv2 实现并进行了定制以支持 RF-DETR 的需求（例如窗口化、注册器 registers、drop-path 等）。

结构概览：
    - 配置类：`WindowedDinov2WithRegistersConfig`（继承自 PretrainedConfig / BackboneConfigMixin）
    - 模型类：Windowed DINOv2 的主干实现（含注册器、窗口注意力、Forward/output 封装）
    - 辅助工具：剪枝 / 激活映射 / 输出索引对齐等工具函数

使用说明：
    - 该文件为复杂的骨干实现，通常通过 `build_backbone` 或 `Joiner` 在上层被封装并与位置编码拼接后作为检测器的输入。

注意：仅在文件顶部添加中文说明注释，不修改任何模型实现或行为。
"""
