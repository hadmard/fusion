...
"""
文件说明：
    对 DINOv2 主干的简易封装，提供根据模型尺度构建 `DinoV2` 类并支持加载 HuggingFace 的预训练权重或 windowed-attention 变体。

结构概览：
    - 配置读取（根据 `size` 与 `use_registers` 加载本地 JSON 配置）
    - `DinoV2` 类：封装了 encoder 的创建、输出特征索引与可选的 windowed-attention 支持
