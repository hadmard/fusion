...
"""
文件说明：
    本模块实现用于实例分割/掩码预测的分割头（SegmentationHead）及其相关辅助模块，如深度可分离卷积块和 MLPBlock。

结构概览：
    - `DepthwiseConvBlock`：深度可分离卷积与 LayerNorm 组合的轻量块
    - `MLPBlock`：用于 query 特征交互的前馈网络块
    - `SegmentationHead`：分割头顶层实现，支持空间特征与查询特征交互，并导出支持 JIT/ONNX 的 `export` 接口

注意：此处仅增强注释，不修改任何实现逻辑。
"""
