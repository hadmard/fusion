...
"""
文件说明：
    本模块实现 LW-DETR（Light-Weight DETR）模型与损失（criterion）相关类，包含检测主干、transformer 集成、分割头接口以及匹配器的调用。

结构概览：
    - 导入与依赖：backbone 构建器、matcher、segmentation head、transformer 等
    - `LWDETR` 主模型类：封装 backbone、transformer、检测头与前向逻辑
    - 损失与后处理工具：与 `build_matcher`、`SegmentationHead` 协同工作

用途说明：
    - `LWDETR` 为模型实现核心，其他上层封装（如 `rfdetr.detr.RFDETR`）会调用本模块来构建具体模型。

注意：仅添加中文说明注释，不修改实现逻辑。
"""
import copy
import math
