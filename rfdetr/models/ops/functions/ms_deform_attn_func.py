...
"""
文件说明：
    本模块为 ms-deformable-attention 的纯 PyTorch 参考实现（主要用于调试与单机测试），核心函数 `ms_deform_attn_core_pytorch` 在 CUDA/扩展不可用时作为回退实现。

结构概览：
    - `ms_deform_attn_core_pytorch`：多尺度可变形注意力的 PyTorch 实现（基于 `grid_sample` 的插值）

注意：仅增加注释，不改动函数实现。该文件通常在高性能场景下被 CUDA 扩展替代。
"""
