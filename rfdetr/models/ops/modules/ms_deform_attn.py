...
"""
文件说明：
    本模块实现多尺度可变形注意力（MSDeformAttn），是 Deformable DETR 家族中常用的高效注意力变体。

结构概览：
    - 核心类 `MSDeformAttn`：定义采样偏移、注意力权重与投影层，并在 forward 中调用后端实现（CUDA 或 PyTorch 回退）
    - 辅助函数 `_is_power_of_2`：检查头维度是否为 2 的幂（CUDA 实现对该情况更高效）

注意：此处仅添加注释，不改动模块实现逻辑。
"""
