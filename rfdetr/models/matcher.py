...
"""
文件说明：
    本模块实现预测与目标之间的匹配器（Hungarian Matcher），用于在训练中为每个样本找到最佳的一对一预测-目标匹配，以便计算损失。

结构概览：
    - 匹配代价计算（类别代价、边界框 L1 代价、GIoU 代价）
    - 可选的 mask 相关代价（dice / bce）和位置调制选项
    - `HungarianMatcher` 类实现具体的线性和分组匹配逻辑，并返回 index 对
