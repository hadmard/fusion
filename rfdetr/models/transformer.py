...
"""
文件说明：
    本模块实现 DETR 中用于检测的 Transformer 结构（encoder/decoder、注意力模块、FFN 等），并包含可用于位置嵌入与查询交互的实用类（例如 `MLP`）。

结构概览：
    - 基础工具类：`MLP`（前馈网络）
    - 位置嵌入/编码生成器（`gen_sineembed_for_position`）
    - Transformer 主体实现（Encoder / Decoder / Attention 变体）

注意：此处增加中文说明而不修改任何模型逻辑实现。
"""
import copy
import math
