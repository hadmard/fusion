...
"""
文件说明：
    本模块实现 Transformer 所需的位置编码（positional encodings），包括基于正弦的位置编码和可学习的位置编码，支持图像输入的二维位置编码。

结构概览：
    - `PositionEmbeddingSine`：基于正弦/余弦的二维位置编码，类似于原始 Transformer 中的实现并扩展到图像。
    - `PositionEmbeddingLearned`：学习型绝对位置嵌入（Embedding）实现。

使用说明：
    - 这些编码在 Transformer encoder/decoder 中作为位置条件输入使用，支持导出模式（`export`）以便 JIT/ONNX 转换。
