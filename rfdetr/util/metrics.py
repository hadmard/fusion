...
"""
文件说明：
    本模块实现训练过程中指标记录与可视化后端适配（Matplotlib、TensorBoard、WandB）。

结构概览：
    第一部分：导入依赖并设置绘图库（Matplotlib）
    第二部分：指标安全访问与工具函数（`safe_index`）
    第三部分：指标 Sink 实现（`MetricsPlotSink` 等），用于保存训练/验证曲线

注意：修改仅为增加中文说明注释，不改变任何函数或逻辑实现。
"""
