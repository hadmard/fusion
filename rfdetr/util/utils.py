...
"""
文件说明：
    本模块提供若干训练/推理时的实用工具类与方法，例如模型指数移动平均（`ModelEma`）、最佳指标跟踪器（`BestMetricSingle`）等。

结构概览：
    第一部分：导入依赖
    第二部分：模型 EMA 与权重管理工具（`ModelEma`）
    第三部分：最佳指标/多指标管理类（`BestMetricSingle`、`BestMetricHolder`）

说明：此修改仅添加中文注释，不改变任何实现逻辑。
"""
