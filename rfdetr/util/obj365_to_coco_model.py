...
"""
文件说明：
    本模块实现将 Object365 预训练权重映射到 COCO 类别索引的辅助函数，便于使用 Object365 预训练模型对 COCO 风格检测头进行微调。

结构概览：
    - 数据映射表（注释形式）
    - `get_coco_pretrain_from_obj365`：根据预定义索引将 obj365 权重重排为 COCO 顺序

注意：仅添加中文头部说明，保留原始实现与注释。
"""
import torch
