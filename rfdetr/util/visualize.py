...
"""
文件说明：
    本模块提供用于将模型检测结果可视化并保存为图片的辅助函数，便于对比 Ground Truth 与预测结果、生成报告图像等。

结构概览：
    第一部分：导入依赖（PIL / NumPy / supervision）
    第二部分：可视化保存函数（如 `save_gt_predictions_visualization`）

注意：本次修改仅增加中文说明注释，未改动任何函数实现或逻辑。
"""
