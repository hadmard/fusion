...
"""
文件说明：
    本模块实现边界框操作相关的工具函数（坐标转换、IoU/GIoU、mask->boxes、DICE/Loss 等），这些函数被检测头与评估代码直接调用。

结构概览：
    - 坐标变换函数：`box_cxcywh_to_xyxy`、`box_xyxy_to_cxcywh`
    - IoU / GIoU 计算：`box_iou`、`generalized_box_iou`
    - Mask/boxes 工具：`masks_to_boxes`
    - 损失工具：`batch_dice_loss`、`batch_sigmoid_ce_loss`
