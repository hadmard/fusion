...
"""
文件说明：
    本模块实现若干常用工具函数，训练与评估流程中广泛使用，包含：
        - 分布式训练辅助（进程同步、reduce 等）
        - 平滑数值统计（`SmoothedValue`）与日志记录工具（`MetricLogger`）
        - 时间与文件相关的实用函数

结构概览：
    第一部分：导入依赖
    第二部分：平滑统计类（`SmoothedValue`）和度量记录器（`MetricLogger`）
    第三部分：分布式同步/辅助函数（`is_dist_avail_and_initialized` 等）
    第四部分：其他工具（时间格式化、文件操作等）
