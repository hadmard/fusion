...
"""
文件说明：
    本文件实现训练与评估的核心函数（供 `main.py` 调用）：
        - `train_one_epoch`：单轮训练循环，实现 AMP、梯度累积、多模态兼容、动态 DropPath/Dropout 调度等功能
        - 评估相关工具（CocoEvaluator 调用与指标计算辅助函数）

结构概览：
    第一部分：导入依赖与类型定义
    第二部分：AMP 配置与辅助函数（`get_autocast_args`）
    第三部分：训练循环（`train_one_epoch`）
    第四部分：评估 / 辅助函数（阈值扫描等工具函数）

代码段说明：
    - 训练循环兼容单模态与双模态数据（根据批次长度判断是否存在 `samples_uv`）。
    - 动态正则化（DropPath / Dropout）通过外部调度表应用到模型实例上。
    - 该文件仅增加注释说明，不修改任何训练或评估逻辑。
"""
import math
import random
...
def train_one_epoch(
    ...
    for data_iter_step, batch in enumerate(
        ...
        for i in range(args.grad_accum_steps):
            ...
            if samples_uv is not None:
                new_samples_uv_tensors = samples_uv.tensors[start_idx:final_idx]
                new_samples_uv = NestedTensor(new_samples_uv_tensors, samples_uv.mask[start_idx:final_idx])
                new_samples_uv = new_samples_uv.to(device)
            else:
                new_samples_uv = None
            new_targets = [{k: v.to(device) for k, v in t.items()} for t in targets[start_idx:final_idx]]

            with autocast(**get_autocast_args(args)):
                if new_samples_uv is None:
                    outputs = model(new_samples, new_targets)
                else:
                    outputs = model(new_samples, new_samples_uv, new_targets)
    ...
    for batch in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with autocast(**get_autocast_args(args)):
            if samples_uv is None:
                ...
                outputs = model(samples, samples_uv)
