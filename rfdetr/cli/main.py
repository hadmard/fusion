...
"""
文件说明：
    命令行工具入口，用于快速从 Roboflow 或本地 COCO 目录启动训练流程（主要为快捷脚本，非主训练逻辑）。

结构概览：
    - `download_dataset`：从 Roboflow API 下载数据集（COCO 格式）
    - `train_from_rf_project` / `train_from_coco_dir`：封装训练入口
    - `trainer`：命令行参数解析与主流程

注意：此文件为 CLI 脚本，仅添加中文说明，不修改功能逻辑。
"""
