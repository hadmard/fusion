    ...
    def _build_coco_dataset(self) -> dict:
        ...
        for img_id in range(len(self.sv_dataset)):
            ...
            for i in range(len(detections)):
                bbox_x, bbox_y, bbox_w, bbox_h = _xyxy_to_xywh(detections.xyxy[i:i+1])[0]
...
class YoloDetection(VisionDataset):
    ...
    def __init__(
        ...
        if sv is None:
            raise ImportError(
                "supervision is required for YOLO datasets; please install a compatible supervision/opencv/numpy stack."
            )
