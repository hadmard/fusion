...
- 额外前置说明：`kimi.pth` 是最新的六层 `attention_residual` 无门控 cross-attention 融合版本；`menkong.pth` 是早期带门控的 cross-attention 融合版本；`uv_single.pth` 是仅使用紫外光输入的单模态版本。
...
- `Precision`
  ...
  - 取总体 `F1` 最大时对应的最佳 confidence threshold。
  - 在该阈值下统计 `TP`、`FP`、`FN`，并按 `TP / (TP + FP)` 计算。

- `Recall`
  - 与 `Precision` 使用同一个最佳 confidence threshold。
  - 固定 `IoU=0.5`。
  - 按 `TP / (TP + FN)` 计算。

- `F1-Score`
  - 与 `Precision`、`Recall` 使用同一个最佳 confidence threshold。
  - 固定 `IoU=0.5`。
...
- `FPS`
  ...
  - 每次正式计时时：
    - 若使用 CUDA，前向前先 `torch.cuda.synchronize()`
    - 使用 `time.perf_counter()` 记录开始时间
    - 执行一次完整前向推理
    - 若使用 CUDA，前向后再次 `torch.cuda.synchronize()`
...
- 单模型结果：
  - `output/multi_checkpoint_eval/2026-03-24_174549/kimi/summary_report.json`
  - `output/multi_checkpoint_eval/2026-03-24_174549/menkong/summary_report.json`
  - `output/multi_checkpoint_eval/2026-03-24_174549/uv_single/summary_report.json`
